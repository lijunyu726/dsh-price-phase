 ...
 */
export function pricePhase(now) {
 ...
 */
export function msUntilNextChange(now) {
  const t = typeof now === 'number' ? now : now.getTime()
...
export const PEAK_COLOR = '#D9A24A'
/** 空闲时段使用的强调色（与 DSH 主题的成功色一致）。 */
export const OFFPEAK_COLOR = '#57C07C'
 ...
 */
export function describePhase(now) {
  const t = typeof now === 'number' ? now : now.getTime()
